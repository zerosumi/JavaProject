...
public class OutputThreadMap {  
    private HashMap<User, OutputThread> map;  
    private static OutputThreadMap instance;  
  
    // 私有构造器，防止被外面实例化改对像  
    private OutputThreadMap() {  
        map = new HashMap<User, OutputThread>();  
    }  
    ...
    // 添加写线程的方法  
    public synchronized void add(User id, OutputThread out) {  
        map.put(id, out);  
    }  
  
    // 移除写线程的方法  
    public synchronized void remove(User id) {  
        map.remove(id);  
    }  
  
    // 取出写线程的方法,群聊的话，可以遍历取出对应写线程  
    public synchronized OutputThread getById(User id) {  
        return map.get(id);  
    }  
    ...
    public synchronized List<OutputThread> getAll() {  
        ...
        for (Map.Entry<User, OutputThread> entry : map.entrySet()) {  
